...
export const registerUser = async (req,res) =>{
    try{
        const {first_name, middle_name, last_name,date_of_birth,gender,contact_info, address_line,hospital_id,email,
            username,role_id, employee_id,
            license_number,license_expiry,department,specialization,provider_id} = req.body;
        ...
        if(userExist.rows.length > 0){
         ...
         const userNameTaken = await pool.query('SELECT * FROM users where username = $1',[username])
        ...
        if (role_id === 3){ 
            const providerExist = await pool.query('SELECT * FROM healthcare_providers WHERE license_number = $1 ' , [ license_number]);
        ...
        const newUser = await pool.query( `INSERT INTO users 
            (first_name, middle_name, last_name, date_of_birth,
                ...
                password_hash, hospital_id, role_id, employee_id, department) 
